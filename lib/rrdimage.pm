...
COMMENT:\n'));
    ...
    elsif ($args{type} eq "sensors")
    {
      ...
      if (my $gpu = $extras->{gpu})
      {
        my $namedelta=1+$maxlen-length('gpu');
        my $spacer=" " x ($namedelta);

        push @rrdargs,("DEF:tgpu=$rrdf:$gpu:AVERAGE",
                       "LINE1:tgpu#51CAAB:gpu",
                       "COMMENT:$spacer",
                       "GPRINT:tgpu:MIN:min\\: %4.1lf",
                       "GPRINT:tgpu:MAX:max\\: %4.1lf",
